...
$(document).ready(function () {
    ...
    if ($("#app-invoice-table").length) {
        $("#app-invoice-table").DataTable({
            ...
            responsive: {
                // options for responsive behaviour
                details: {
                    type: "column",
                    target: 0,
                },
    ...
    $(document).on("click", ".discount-apply-btn", function (e) {
        discount_value =
            $(this).parents().eq(2).find("#applicable-discount").val() + "%";
        ...
        if (discount_value || tax_one_val || tax_two_val) {
            var elems = $(this)
                ...
                .eq(4)
                .prev()
                .find(".discount-value, .tax-1-value, .tax-2-value")
                .toArray();
    ...
    $(document).on("change", "#item-options", function () {
        ...
        if (option_selected === "frest") {
            description_input.val(
                "The most developer friendly & highly customisable HTML5 Admin"
            );
        } else if (option_selected === "modern") {
            description_input.val(
                "The most complete & feature packed bootstrap 4 admin template of 2019!"
            );
        } else if (option_selected === "apex") {
            description_input.val(
                "Developer friendly and highly customizable Angular 7+ jQuery Free Bootstrap 4 gradient ui admin template. "
            );
        } else if (option_selected === "stack") {
            description_input.val(
                "Ultimate Bootstrap 4 Admin Template for Next Generation Applications."
            );
