...
class User extends Authenticatable
{
    ...
    protected $fillable = ['name', 'firstname', 'user_profile', 'lastname', 'email', 'username', 'password', 'phone_number', 'interests', 'is_verify', 'is_active'];
    ...
    public function getMyInterestsAttribute()
    {
        ...
        if (is_array($interests) && !empty($interests)) {
            $data = Interest::select('id', 'interest_name', 'featured_image', 'icon', 'background_color')->whereIn('id', $interests)
                ->get()
                ->toArray();
            if (!empty($data)) {
                return $data;
