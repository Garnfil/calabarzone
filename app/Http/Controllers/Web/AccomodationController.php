...
class AccomodationController extends Controller
{
    public function list(Request $request) {
        if($request->ajax()) {
            $data = Accomodation::latest()->with('province', 'city_municipality');
    ...
    public function store(CreateAccomodationRequest $request) {
        ...
        $create = Accomodation::create(array_merge($data, [
            'featured_image' => $featured_image_name,
        ]));

        if($create) return redirect()->route('admin.accomodations')->with('success', 'Accomodation Created Successfully');
    }

    public function edit(Request $request) {
        $accomodation = Accomodation::where('id', $request->id)->firstOrFail();
        $provinces = Province::get();
        $interests = Interest::get();
        return view('admin-page.accomodations.edit', compact('accomodation', 'provinces', 'interests'));
    }

    public function update(UpdateAccomodationRequest $request) {
        $data = $request->validated();
        $accomodation = Accomodation::where('id', $request->id)->firstOrFail();
        ...
        if($update) return back()->with('success', 'Accomodation Updated Successfully');
    }
}
